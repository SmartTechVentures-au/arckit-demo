...
**Link to risk register**: **Corrected 2026-07-30.** The original text of this line asserted that no `ARC-001-RISK-v*.md` existed. That was wrong — `ARC-001-RISK-v1.0.md` has existed since 2026-07-27 with 29 registered risks, and eight other ADRs cite it correctly (HLDR BLOCKING-02). The twelve risks above have now been **migrated into the register as R-030 to R-041**, mapped in order (`RISK-A1` → R-030 … `RISK-A12` → R-041). The `RISK-A*` identifiers are retained in each register entry's description for traceability back to this section. The register records existing controls as *None* and residual equal to inherent for all twelve, because this ADR is status Proposed and Rules 1-8 are not yet in force.
...
**Risk register**: `ARC-001-RISK-v1.0.md` (created 2026-07-27). `RISK-A1`–`RISK-A12` (§7.4) were migrated into it as **R-030 to R-041** on 2026-07-30, retaining the `RISK-A*` identifiers in each entry's description. The earlier claim in this document that the register did not exist was incorrect — see §7.4 and HLDR BLOCKING-02.
...
| Build Wave | 1 |
| Build Target | `ADR-AI` |
| Build Topic | AI and ML services in the learning ecosystem |
| Stamped at | 2026-07-30T00:28:01.862Z |
